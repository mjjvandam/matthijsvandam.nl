...
function isNewsletter(campaign) {
  return NEWSLETTER_NAME.test(String(campaign.name || '')) || String(campaign.tag || '').toLowerCase() === 'mvd-nieuwsbrief';
}
...
async function readBrevoNewsletterHistory({apiKey,baseUrl='https://api.brevo.com/v3',limit=500,fetchImpl=fetch}={}) {
  ...
  return {historyComplete:payload.count == null || campaigns.length < limit || payload.count <= limit,historyCheckedAt:new Date().toISOString(),history:campaigns.map(normalizeCampaign)};
}
