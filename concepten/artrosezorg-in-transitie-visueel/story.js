...
(() => {
  ...
  function closeMenu() { siteNav.hidden=true; menuToggle.setAttribute('aria-expanded','false'); }
  closeMenu();
  menuToggle.hidden=false;
  menuToggle.addEventListener('click',()=>{
    const open=menuToggle.getAttribute('aria-expanded')!=='true';
    siteNav.hidden=!open;menuToggle.setAttribute('aria-expanded',String(open));
  });
  document.addEventListener('keydown',event=>{
    if(event.key==='Escape' && !siteNav.hidden){closeMenu();menuToggle.focus();}
  });
